...
class EmployeeCornerController extends Controller
{
    ...
    public function viewAddEmployeeco()
    {
        if (!empty(Session::get("emp_email"))) {
            ...
            if(Session::get("users_id_new")){
                $arrayCheck[]= Session::get("users_id_new");
            }else{
                 $arrayCheck[]= Session::get("users_id");
            }
            $userId=implode(",",$arrayCheck);
    ...
    public function viewdetaholiday()
    {
        if (!empty(Session::get("emp_email"))) {
            ...
            if($user_type==="employer"){
                $arrayCheck[] =$users->employee_id;
            }else{
                $arrayCheck[] =$users->emid;
            }
            $userId=implode(",",$arrayCheck);
    ...
    public function viewapplyleaveapplication()
    {
        if (!empty(Session::get("emp_email"))) {
            $user_id = Session::get("users_id");
            $user_type=Session::get("user_type");
            $users_id_new=Session::get("users_id_new");
        ...
        //    dd($users->employee_id);
            ...
            $employee = Employee::where("emid", "=", $users->employee_id)
                ->orWhere("emp_code", "=", $users->employee_id)
                ->first();
            // dd($users->employee_id);
            ...
            $leave_type_rs = LeaveType::join(
                ...
                ->where("leave_type.emid", "=",$userId)
                ->where(
                    "leave_allocation.employee_code",
                    "=",
                    $users->employee_id
                )
                ->where("leave_allocation.emid", "=", $userId)
