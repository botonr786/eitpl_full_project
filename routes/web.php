...
Route::get('register', 'App\Http\Controllers\LandingController@register');
...
//TASK Perfomance management Start
...
//TASK $ PERFOMANCE MANAGEMENT END
	...
	<div class="col-md-4">
					<div class="form-group">
						...
						<select class="form-control input-border-bottom"   id="package' . $row . '" required=""  name="package[]" style="margin-top: 22px;" onchange="checkpackage(this.value,' . $row . ');">
											...
											<button class="btn btn-success cmn_bg_style" type="button" id="add' . ($row + 1) . '" onClick="addnewrow(' . ($row + 1) . ')" data-id="' . ($row + 1) . '"> <i class="fas fa-plus"></i> </button>
...
// Use for loop to store dates into array
    ...
    if (trim($leave_tyepenew->alies) == 'HOLIDAY' || trim($leave_tyepenew->alies) == 'H' || trim($leave_tyepenew->alies) == 'Holiday') {
...
onchange="getreviewnatdateother(' . $row . ');">
		 ...
		 <div class="col-md-4" style="margin-top:27px;"><button class="btn-success cmn_bg_style" type="button"  id="adduploadother' . $row . '" onClick="addnewrowuploadother(' . $row . ')" data-id=" ' . $row . '"><i class="fas fa-plus"></i> </button>
...
Route::get('settings/get-add-row-item/{row}', function ($row) {
    ...
    $result = ' <tr class="itemslot" id="' . $row . '" >
					    <td>' . $row . '</td>
						 ...
						 <td><button class="btn-success cmn_bg_style" type="button" id="add' . $row . '" onClick="addnewrow(' . $row . ')" data-id="' . $row . '"> <i class="fas fa-plus"></i> </button>
...
Route::get('settings/get-add-row-acc/{row}', function ($row) {
    $row = $row + 1;
    $response=DB::table('education_master')->get();

    $result = ' <tr class="itemslot" id="' . $row . '" >
    <td>' . $row . '</td>
    <td>

    <select class="form-control" name="emp_document_name[]" id="qualification' . $row . '">

    <option value="" selected>Select</option>';

                foreach ($response as $educ) {

                $result .= '<option value="' . $educ->education_name . '">' . $educ->education_name . '</option>';
                }
...
<button class="btn-success cmn_bg_style" type="button"  id="addedu' . $row . '" onClick="addnewrowedu(' . $row . ')" data-id="' . $row . '"><i class="fas fa-plus"></i> </button>
...
Route::get('settings/get-add-row-item-train/{row}', function ($row) {
    ...
    $result = '
				  ...
				  <div class="col-md-4">
					...
					<div class="col-md-4" style="margin-top:27px;"><button class="btn-success cmn_bg_style" type="button" id="addtarin' . $row . '" onClick="addnewrowtarin(' . $row . ')" data-id="' . $row . '"><i class="fas fa-plus"></i> </button>

					 <button class="btn-danger cmn_bg_style deleteButtontrain" type="button" id="deltarin' . $row . '"  onClick="delRowtrain(' . $row . ')"> <i class="fas fa-minus"></i> </button></div>
				  </div>
                 </div></br>	  ';
    echo $result;
});
...
Route::get('settings/get-add-row-item-upload/{row}', function ($row) {
    ...
    $result = '
				  ...
				  <div class="col-md-4" style="margin-top:27px;"><button class="btn-success cmn_bg_style" type="button"  id="addupload' . $row . '" onClick="addnewrowupload(' . $row . ')" data-id=' . $row . '"><i class="fas fa-plus"></i> </button>
...
onchange="getreviewnatdateother(' . $truotherdocpload_id . ');">
            ...
            if ($truotherdocpload_id == ($countpayuppasother)) {
                $reslt .= '<div class="col-md-4" style="margin-top:27px;"><button class="btn-success cmn_bg_style" type="button"  id="adduploadother' . $truotherdocpload_id . '" onClick="addnewrowuploadother(' . $truotherdocpload_id . ')" data-id="' . $truotherdocpload_id . '"><i class="fas fa-plus"></i> </button></div>';
...
onchange="getreviewnatdateother(' . $truotherdocpload_id . ');">
		 ...
		 <div class="col-md-4" style="margin-top:27px;"><button class="btn-success cmn_bg_style" type="button"  id="adduploadother' . $truotherdocpload_id . '" onClick="addnewrowuploadother(' . $truotherdocpload_id . ')" data-id="' . $truotherdocpload_id . '"><i class="fas fa-plus"></i> </button></div>
...
Route::post('role/vw-user-config-update','App\Http\Controllers\UseraceesController@updateUserConfig');
// role/vw-user-config-update
